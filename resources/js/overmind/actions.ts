...
import { Gist, Settings, Folder } from '../types';
...
export const addOrRemoveFromFolder = ({ state }: Context, folderID: number) => {
  ...
  const selectedFolder = folders.findIndex(
    (folderObject) => folderObject.id === folderID
  );
...
export const toggleSelected = (
  ...
  if (shift && selected.length) {
    ...
    const firstID =
      gistKeys[
        Math.min(gistKeys.indexOf(id), gistKeys.indexOf(state.lastSelected))
      ];
    const lastID =
      gistKeys[
        Math.max(gistKeys.indexOf(id), gistKeys.indexOf(state.lastSelected))
      ];
