    ...
    <div className="relative select-none">
      ...
      <div className="text-left text-sm leading-[24px]">
        {(state.addFolder || ![0, 1].includes(state.editFolder)) && (
          <div>
            <div className="flex">
              <input
                type="text"
                className="border text-gray-700 border-gray-700 px-2 w-full focus:outline-none"
                ref={folderName}
                defaultValue={title}
              />
              <button
                className="text-white bg-gray-700 px-4 focus:outline-none"
        ...
        <div className="flex justify-between text-xs">
          ...
          {![0, 1].includes(state.folder) &&
            [0, 1].includes(state.editFolder) && (
              <>
                <span
                  className="text-gray-500 rounded-full flex items-center cursor-pointer"
                  onClick={editFolder}
                  onMouseEnter={() => setFolderEdit(true)}
                  onMouseLeave={() => setFolderEdit(false)}
                >
                ...
                >
                  ...
                  <span
                    className="ml-1"
                    onMouseEnter={() => setFolderDelete(true)}
                    onMouseLeave={() => setFolderDelete(false)}
                  >
                    Delete Folder
                  </span>
                </span>
