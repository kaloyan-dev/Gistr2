          ...
          state.folders.map((folder, index) => {
            if (folder.gists.includes(gist.id)) {
              ...
              return (
                <span
                  key={index}
                  className={`${bgColor} ml-2 text-white leading-none py-1 px-2 rounded-md text-xs`}
                >
                  {folder.title}
                </span>
              );
    ...
    <div className="shadow mt-4 w-full relative">
      ...
      <ul>
        {gists.filtered.map((gist, id) => {
          ...
          return (
            <li
              className={`${bgColor} first:mt-0 cursor-pointer border-b last:border-b-0 select-none`}
              key={id}
            >
              <div className={`${checkboxClass} relative`}>
                ...
                <div className="flex justify-between">
                  <div
                    className={`${padding} flex items-center w-full transition-all`}
                    onClick={() => toggleOpen(gist.id)}
                  >
                    <h2 className={titleColor}>{gistTitle(gist)}</h2>
                  </div>
                  <div
                    className={`${padding} flex text-gray-400 transition-all cursor-default`}
                  >
                    <a
                      href={getGistURL(gist)}
                      target="_blank"
                    ...
                    <a
                      href={getGistURL(gist, '/edit')}
                      target="_blank"
                      className="ml-3 hover:text-gray-700"
                    >
                      <Icon
                        type="pencil"
                    ...
                    <span
                      className="ml-3 hover:text-gray-700"
                      onClick={() => copyToClipboard(gist)}
                      onMouseEnter={() => setCopied('')}
                    >
                      <Icon
                        type={
                          copied === gist.id ? 'clipboard-check' : 'clipboard'
                        }
                    ...
                    <span
                      className="ml-3 hover:text-green-500"
                      onClick={() => toggleFavorite(gist.id)}
                    >
                      <Icon
                        type={starIcon}
